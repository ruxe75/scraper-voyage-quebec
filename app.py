...
KEYWORDS = ["Breathless", "Riu", "Hard Rock", "Royalton CHIC", "Excellence", "TRS", "Barcel", "Temptation", "Secrets", "Hyatt Zilara"]
...
def generate_pdf(deals):
    ...
    for d in deals:
        ...
        pdf.multi_cell(0, 6, f"Party:{d.party_score} | Food:{d.food_score} | Drinks:{d.drinks_score}")
        if d.link:
...
if st.button("Fetch Latest Deals"):
    with st.spinner("Fetching deals... please wait (20-40 sec)"):
        deals = get_all_deals()
    if not deals:
        ...
        table = []
        for d in deals:
            table.append([d.provider, d.title, f"${d.price}", d.departure_city,
                          d.party_score, d.food_score, d.drinks_score, d.link])
        st.table(table)
        pdf_data = generate_pdf(deals)
        st.download_button("📄 Download PDF Report", data=pdf_data,
                           file_name="vacation_deals.pdf", mime="application/pdf")
else:
    st.info("Click 'Fetch Latest Deals' to get updated offers.")
